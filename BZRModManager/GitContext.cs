...
namespace BZRModManager
{
    class GitContext
    {
        public static async Task<string[]> GetModBranchesAsync(string gitExePath, string url)
        {
        ...
        public static async Task WorkshopDownloadItemAsync(string gitExePath, UInt32 appId, string url, string[] branches)
        {
            ...
            foreach (string branch in branches)
            {
                ...
                if (!Directory.Exists(outputDir))
                {
                    ...
                    await CheckoutAsync(gitExePath, url, branch, Path.GetFullPath(outputDir));
                }
            }
        }

        private static async Task CheckoutAsync(string gitExePath, string url, string branch, string outputDir)
        {
            //if (!Directory.Exists(outputDir))
            {
                ProcessStartInfo info = new ProcessStartInfo()
                ...
                info.EnvironmentVariables.Add("GIT_TERMINAL_PROMPT", "0");
                Process git = Process.Start(info); while (git.HasExited == false) await Task.Delay(100);

                // create a local repo with the needed branch
                info.Arguments = $"clone checkout -b {branch}";
                git = Process.Start(info); while (git.HasExited == false) await Task.Delay(100);
                ...
                //}
                await PullAsync(gitExePath, outputDir);
            }
        }
        ...
        public static async Task PullAsync(string gitExePath, string gitPath)
        {
            ProcessStartInfo info = new ProcessStartInfo()
            {
                //FileName = "git.exe",
